...
<xsl:stylesheet xmlns:default="http://www.loc.gov/mods/v3"
    ...
    <!--   
    This stylesheet splits MODS XML files created by tokens2mods.xsl into separate MODS files 
    ...
    <xsl:template match="/">
        <xsl:for-each select="/*/*/*[not(. > 3)]">
            <!-- saxon:output href="Y:\xslt-output\mods-output\mods\{./default:identifier}.xml" -->
            <!-- <saxon:output href="J:\techSpace\xslt\xslt-output\mods-output\mods\{./default:identifier}.xml"> -->
                <saxon:output href="\\150.209.8.28\f$\techSpace\xslt\xslt-output\mods-output\mods\{./default:identifier}.xml"> 
                    <xsl:copy-of select="."/>
            </saxon:output>
        </xsl:for-each>
    </xsl:template>
    ...
    <!-- EXPERIMENTAL CODE 
$dir = default:identifier;
create_dir($dir);

$source = concat($dir.xml);
$target = concat($dir, '/', $dir.xml);
copy($source, $target)
-->
